...
function List() {
  ...
  async function getData() {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/operator-list`, {
        method: "GET",
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/operator-delete/${id}`, {
        method: "DELETE",
        headers: { 'authorization': `Bearer ${token}` },
      });
  ...
  const handleActiveChange = async (id, isActive) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/operator-update-active/${id}`, {
        method: "PUT",
      ...
      if (response.ok) {
        toastr.success("Operator activation status updated successfully");
        getData();
      } else {
        toastr.error(result.error);
      }
        ...
        cell: ({ row }) => (
          <div className="d-flex" style={{ textAlign: "center" }}>
            <span type="button" className="btn btn-outline-light">
              <Link to={`/operators/operator-update/${row.original._id}`} className="card-link" >
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      return (
        row.company.toString().toLowerCase().includes(lowercasedFilter) ||
