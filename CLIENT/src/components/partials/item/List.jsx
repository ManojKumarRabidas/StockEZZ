...
function List() {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/item-delete/${id}`, {
        method: "DELETE",
        headers: { 'authorization': `Bearer ${token}` },
      });
  ...
  const handleActiveChange = async (id, isActive) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/item-update-active/${id}`, {
        method: "PUT",
        ...
        cell: ({ row }) => (
          <div className="d-flex align-items-center justify-content-center" style={{ textAlign: "center" }}>
            <span type="button" className="btn btn-outline-light">
              <Link to={`/items/item-update/${row.original._id}`} disabled className="card-link" >
