...
function List() {
  ...
  async function getData() {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/support-admin-list`, {
        method: "GET",
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/support-admin-delete/${id}`, {
        method: "DELETE",
        headers: { 'authorization': `Bearer ${token}` },
      });
      ...
      const result = await response.json();
      if (response.ok) {
        toastr.success("Support Admin deleted successfully");
        getData();
      } else {
  ...
  const handleActiveChange = async (id, isActive) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/support-admin-update-active/${id}`, {
        method: "PUT",
      ...
      if (response.ok) {
        toastr.success("Support Admin activation status updated successfully");
        getData();
      } else {
        toastr.error(result.error);
      }
        ...
        cell: ({ row }) => (
          <div style={{ textAlign: "center" }}>
            <button type="button" className="btn btn-outline-light m-1" style={{ backgroundColor: "ghostwhite" }}>
              <Link to={`/support-admins/support-admin-update/${row.original._id}`} className="card-link m-2" >Edit</Link>
            </button>
