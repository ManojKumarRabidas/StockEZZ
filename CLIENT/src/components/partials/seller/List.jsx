...
function List() {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/seller-delete/${id}`, {
        method: "DELETE",
        headers: { 'authorization': `Bearer ${token}` },
      });
  ...
  const handleActiveChange = async (id, isActive) => {
    try {
      const response = await fetch(`${VITE_API_BASE_URL}/server/seller-update-active/${id}`, {
        method: "PUT",
      ...
      if (response.ok) {
        toastr.success("Seller activation status updated successfully");
        getData();
      } else {
        toastr.error(result.error);
      }
        ...
        cell: ({ row }) => (
          <div className="d-flex justify-content-center" style={{ textAlign: "center" }}>
            <span type="button" className="btn btn-outline-light">
              <Link to={`/sellers/seller-update/${row.original._id}`} className="card-link" >
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      return (
        ...
        row.branch.toString().toLowerCase().includes(lowercasedFilter)
      );
    });
  }, [data, globalFilter]);
